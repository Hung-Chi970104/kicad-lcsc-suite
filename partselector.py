...
class PartSelectorDialog(wx.Dialog):
    ...
    def __init__(self, parent, parts):
        ...
        self.import_lib_button = wx.Button(
            self,
            wx.ID_ANY,
            "Import symbol/fp",
            wx.DefaultPosition,
        ...
        self.import_lib_button.SetToolTip(
            "Download the EasyEDA symbol, footprint and 3D model for the "
            "selected part into the project's LCSC library"
        )
        ...
        self.import_lib_button.Bind(wx.EVT_BUTTON, self.import_selected_library)

        self.import_lib_button.SetBitmap(
            loadBitmapScaled(
                "mdi-database-import-outline.png",
                self.parent.scale_factor,
            )
        )
        self.import_lib_button.SetBitmapMargins((2, 0))
        ...
        tool_sizer.Add(self.import_lib_button, 0, wx.ALL, 5)
    ...
    def import_selected_library(self, *_):
        """Import symbol, footprint and 3D model for the highlighted part."""
        if self.part_list.GetSelectedItemsCount() == 0:
            return
        item = self.part_list.GetSelection()
        lcsc = self.part_list_model.get_lcsc(item)
        ...
        root = self.parent._lcsc_library_root()
        ...
        self.import_lib_button.Disable()
        self.result_count.SetLabel(f"Importing {lcsc} …")

        def work():
            result = importer.import_part(lcsc, overwrite=False)
            actions = importer.register_libraries(project_dir=project_dir) if result.ok else []
            wx.CallAfter(self._import_done, result, actions)

        threading.Thread(target=work, daemon=True).start()

    def _import_done(self, result, actions):
        """Report the outcome of a library import."""
        self.import_lib_button.Enable()
        ...
        wx.MessageBox(
            message,
            "LCSC import",
            style=wx.ICON_ERROR if result.errors else wx.ICON_INFORMATION,
        )
