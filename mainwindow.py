...
class JLCPCBTools(wx.Dialog):
    ...
    def __init__(self, parent, kicad_provider=KicadProvider()):
        ...
        self.lcsc_explorer_button = self.upper_toolbar.AddTool(
            ID_LCSC_EXPLORER,
            "LCSC Explorer",
            loadBitmapScaled("mdi-magnify.png", self.scale_factor),
            "Search LCSC/JLCPCB with parametric filters, compare assembly vs "
            "retail stock, preview and import symbols and footprints",
        )

        self.lcsc_import_all_button = self.upper_toolbar.AddTool(
            ID_LCSC_IMPORT_ALL,
            "Import libs",
    ...
    def _lcsc_library_root(self) -> str:
        ...
        Defaults to a project-local ``lcsc-lib`` so imported symbols travel
        with the board between machines.
        """
        configured = self.settings.get("lcsc", {}).get("library_root")
        if configured:
            return str(configured)
        board_file = self.pcbnew.GetBoard().GetFileName()
        if board_file:
            return os.path.join(os.path.dirname(board_file), "lcsc-lib")
    ...
    def import_all_lcsc_libs(self, *_):
        ...
        parts = self.store.read_all()
        lcsc_numbers = sorted(
            {str(p.get("lcsc") or "").strip() for p in parts if p.get("lcsc")}
        )
        if not lcsc_numbers:
            wx.MessageBox(
                "No LCSC part numbers are assigned on this board yet.",
                "Nothing to import",
                style=wx.ICON_INFORMATION,
            )
            return

        root = self._lcsc_library_root()
        if (
            wx.MessageBox(
                f"Import symbols, footprints and 3D models for {len(lcsc_numbers)} "
                f"part(s) into:\n\n{root}\n\nExisting entries are kept "
                "(nothing is overwritten).",
                "Import LCSC libraries",
        ...
        board_file = self.pcbnew.GetBoard().GetFileName()
        project_dir = os.path.dirname(board_file) if board_file else ""
        importer = LcscImporter(
            root=root,
            lib_name=DEFAULT_LIB_NAME,
            project_relative=is_inside(root, project_dir),
        )

        progress = wx.ProgressDialog(
            "Importing LCSC libraries",
            "Starting…",
            maximum=len(lcsc_numbers),
            parent=self,
            style=wx.PD_APP_MODAL | wx.PD_CAN_ABORT | wx.PD_ELAPSED_TIME,
        )
        succeeded, skipped, failed = [], [], []
        try:
            for index, lcsc in enumerate(lcsc_numbers, start=1):
                keep_going, _ = progress.Update(index - 1, f"{lcsc} …")
                if not keep_going:
                    break
                result = importer.import_part(lcsc, overwrite=False)
                self.logger.info(result.describe())
                if result.errors:
                    failed.append(f"{lcsc}: {'; '.join(result.errors)}")
                elif result.ok:
                    succeeded.append(lcsc)
                else:
                    skipped.append(lcsc)
        finally:
            progress.Destroy()
        ...
        summary = [
            f"Imported: {len(succeeded)}",
            f"Already present: {len(skipped)}",
            f"Failed: {len(failed)}",
        ]
        if failed:
            summary.append("")
            summary.extend(failed[:12])
            if len(failed) > 12:
                summary.append(f"… and {len(failed) - 12} more (see the log).")
        if actions:
            summary.append("")
            summary.extend(actions)
        summary.append("")
        summary.append(
            "KiCad caches library tables at startup — restart KiCad if the "
            "library does not show up in the choosers."
        )
        wx.MessageBox(
            "\n".join(summary), "LCSC library import", style=wx.ICON_INFORMATION
        )
